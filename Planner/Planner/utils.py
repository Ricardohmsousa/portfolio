...
def generate_unique_identifier():
    unique_id = str(uuid.uuid4().hex)[:15]
    return unique_id
...
def getDataFromPDF(prompt):
    model = GenerativeModel("gemini-pro-vision")
    text_meta=pd.read_csv('text_metadata_df_sus.csv')
    image_meta=pd.read_csv('image_metadata_df_sus.csv')


    for i in range(len(text_meta["text_embedding_page"])):
        text_meta.at[i, "text_embedding_page"] = ast.literal_eval(text_meta.at[i, "text_embedding_page"])

    for i in range(len(text_meta["text_embedding_chunk"])):
        text_meta.at[i, "text_embedding_chunk"] = ast.literal_eval(text_meta.at[i, "text_embedding_chunk"])
    for i in range(len(image_meta["mm_embedding_from_text_desc_and_img"])):
        image_meta.at[i, "mm_embedding_from_text_desc_and_img"] = ast.literal_eval(image_meta.at[i, "mm_embedding_from_text_desc_and_img"])

    for i in range(len(image_meta["mm_embedding_from_img_only"])):
        image_meta.at[i, "mm_embedding_from_img_only"] = ast.literal_eval(image_meta.at[i, "mm_embedding_from_img_only"])
    for i in range(len(image_meta["text_embedding_from_image_description"])):
        image_meta.at[i, "text_embedding_from_image_description"] = ast.literal_eval(image_meta.at[i, "text_embedding_from_image_description"])
# Retrieve relevant chunks of text based on the query
    matching_results_chunks_data = get_similar_text_from_query(
        "master-chariot-325508",
        prompt,
        text_meta,
    ...
    a=get_gemini_response(model, model_input=final_prompt)
    print(final_prompt)
    print(a)
    return a
def getRoutes():
...
# Specify the path to your CSV file
    csv_file_path = 'C:\\Users\\ricardo.msousa\\Desktop\\gtfs\\routes.csv'
